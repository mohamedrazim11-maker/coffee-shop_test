...
import { Leaf, Coffee, ArrowRight, Droplets } from 'lucide-react';
import { NAV_LINKS, COFFEE_FLAVORS } from '@/constants/flavors';
...
export default function CoffeeShowcase() {
    ...
    return (
        <div className="relative min-h-screen text-stone-100 font-sans bg-stone-950 selection:bg-emerald-700 selection:text-white">
            ...
            {/* --- HERO SECTION --- */}
            <main className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-12 items-center pt-8 pb-20 gap-16">
                <motion.div
                ...
                >
                    <motion.div variants={fadeIn} className="space-y-6">
                        ...
                        <p className="text-stone-400 text-base md:text-lg max-w-md font-light leading-relaxed mx-auto lg:mx-0">
                            Aura Botanica: Where deep wild nature meets exceptional, micro-roasted coffee crafted with precision ratios.
                        </p>
                    </motion.div>
                ...
                {/* Right Visual Column */}
                <div className="lg:col-span-5 relative flex flex-col items-center justify-center min-h-[420px] w-full">
                    ...
                    <motion.div
                        ...
                        className="absolute -bottom-8 left-4 sm:-left-4 z-20 w-36 h-36 sm:w-44 sm:h-44 drop-shadow-[0_20px_40px_rgba(0,0,0,0.9)] hover:scale-105 transition-transform duration-300"
                    >
            ...
            {/* --- FLAVORS & COFFEE TYPES SECTION --- */}
            <section id="flavors" className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 py-20 border-t border-white/5">
                <div className="text-center max-w-2xl mx-auto mb-16 space-y-3">
                    <span className="text-amber-300/80 text-xs tracking-[0.3em] font-semibold uppercase">Craft Ratios</span>
                    <h2 className="text-3xl md:text-5xl font-serif text-stone-100">Our Coffee Menu & Flavors</h2>
                    <p className="text-stone-400 text-sm font-light">Explore our precise espresso-to-water & milk balances to find your perfect cup.</p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {COFFEE_FLAVORS.map((flavor, index) => (
                        <motion.div
                            key={flavor.name}
                            ...
                            className="relative group bg-stone-900/50 hover:bg-stone-900 border border-white/5 hover:border-emerald-500/40 p-6 rounded-2xl transition-all duration-300 backdrop-blur-sm hover:-translate-y-1 shadow-lg"
                        >
                            {flavor.tag && (
                                <span className="absolute top-4 right-4 bg-emerald-950 border border-emerald-500/30 text-emerald-400 text-[10px] uppercase font-semibold px-2.5 py-0.5 rounded-full">
                                    {flavor.tag}
                                </span>
                            )}

                            <div className="flex items-center space-x-3 mb-3">
                                <div className="p-2 bg-amber-500/10 rounded-xl border border-amber-500/20 text-amber-300">
                                    <Coffee className="w-5 h-5" />
                                </div>
                                <h3 className="text-xl font-serif font-semibold text-stone-100">{flavor.name}</h3>
                            </div>

                            <div className="space-y-2 mt-4">
                                <div className="flex items-center space-x-2 text-xs font-mono text-emerald-400 bg-emerald-950/40 px-2.5 py-1 rounded-md border border-emerald-500/10 w-fit">
                                    <Droplets className="w-3.5 h-3.5 text-emerald-400" />
                                    <span>{flavor.ratio}</span>
                                </div>
